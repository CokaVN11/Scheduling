...
void Scheduler::srtn()
{
    ...
    while (!pq.empty())
    {
        // move to the next process has the arrival time is bigger than cpu_time
        if (pq.top().arrival > cpu_time)
        {
            for (int i = 0; i < pq.top().arrival - cpu_time; ++i)
                cpu_gantt.push_back(-1);
            cpu_time = pq.top().arrival;
        }
